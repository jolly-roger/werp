...
try:
        ...
        if et.ref_id is None:
            if (et.from_date is None and et.to_date is None) or \
                ...
                similar_etrains = ses.query(orm.uatrains.E).\
                    filter(orm.and_(orm.uatrains.E.oid == et.oid, orm.uatrains.E.ua_title == et.ua_title,
                        orm.uatrains.E.ru_title == et.ru_title,
    ...
    exec_log.info('uatrains bot referer %s %s' % (str(start_dt), str(exec_delta)))
except:
    if ses is not None:
        ses.close()
    if conn is not None:
