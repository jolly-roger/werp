...
def get_train_data(tid):
	...
	try:
		...
		ua_res = urllib.request.urlopen(ua_req, timeout=60)
		...
		ru_res = urllib.request.urlopen(ru_req, timeout=60)
		...
		en_res = urllib.request.urlopen(en_req, timeout=60)
...
def get_s(e, ses):
    ...
    try:
        ...
        s = ses.query(orm.E).filter(orm.and_(orm.and_(orm.E.etype == e.etype, orm.E.ua_title.ilike(prepared_ua_title)),
            orm.E.value == e.value)).one()
    except:
