...
def is_added(train_station, ses):
	ret = False
	try:
		tss = ses.query(orm.TrainStation).\
			filter(orm.and_(orm.TrainStation.t_id == train_station.t_id, orm.TrainStation.s_id == train_station.s_id,
				orm.TrainStation.order == train_station.order)).all()
		if len(tss) > 0:
			ret = True
	except:
		pass
...
def is_changed(train_station, ses):
	ret = False
	try:
		ts = ses.query(orm.TrainStation).\
			filter(orm.and_(orm.TrainStation.t_id == train_station.t_id,
