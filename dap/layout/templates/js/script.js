...
$(function(){
    function get_accessibility(){
        $.getJSON( "/get_10_checked?ses_key=" + ses_key + "&domain=" + domain_value, function(data){
            for (var i in data) {
                $('#' + proxy_status_prefix + data[i]['id']).
                    html('<span class="round label accessible_status">{% trans %}accessible_label{% endtrans %}</span>');
            }
        });
        $.getJSON( "/is_check_finished?ses_key=" + ses_key + "&domain=" + domain_value, function(data){
            if(!data){
    ...
    $('#check_proxies_btn').click(function(){
        ...
        if(is_valid){
            $.post('/check_10', {ses_key: ses_key, domain: domain_value, jproxies: jproxies}, function(data){
